...
func (c *Client) SendPasswordResetEmailManyPersonalized(toEmails []string, resetLink string) error {
	uniq := sanitizeAndDeduplicateEmails(toEmails)
	...
	for _, email := range uniq {
		dear := utils.DisplayName(email)
		htmlBody := buildResetEmailHTML(dear, resetLink)

		err := c.SendEmailOnly(email, "Password Reset Request", htmlBody)
		if err != nil {
			return fmt.Errorf("failed to send to %s: %w", email, err)
		}
	}
...
func sanitizeAndDeduplicateEmails(toEmails []string) []string {
