package models

import (
	"time"

	mssql "github.com/microsoft/go-mssqldb"
)

type EDI_ForecastVersionStatusLogResp struct {
	EDIForecastVersionID  mssql.UniqueIdentifier `json:"edi_forecast_version_id"`
	EDIForecastID         mssql.UniqueIdentifier `json:"edi_forecast_id"`
	OldStatus             string                 `json:"old_status"`
	NewStatus             string                 `json:"new_status"`
	Note                  string                 `json:"note"`
	ChangedByExternalID   string                 `json:"created_by_external_id"`
	ChangedBySourceSystem string                 `json:"created_by_source_system"`
	FileURL               *string                `json:"file_url"`
	CreatedAt             time.Time              `json:"created_at"`
}

// ใช้รับข้อมูลจาก multipart/form-data
type EDI_ForecastVersionStatusLogForm struct {
	EDIForecastVersionID  string `form:"edi_forecast_version_id"`
	EDIForecastID         string `form:"edi_forecast_id"`
	OldStatus             string `form:"old_status"`
	NewStatus             string `form:"new_status"`
	Note                  string `form:"note"`
	ChangedByExternalID   string `form:"created_by_external_id"`
	ChangedBySourceSystem string `form:"created_by_source_system"`
}

type EDI_ForecastVersionStatusLogReq struct {
	EDIForecastID         mssql.UniqueIdentifier `json:"edi_forecast_id"`
	OldStatus             string                 `json:"old_status"`
	NewStatus             string                 `json:"new_status"`
	Note                  string                 `json:"note"`
	ChangedByExternalID   string                 `json:"created_by_external_id"`
	ChangedBySourceSystem string                 `json:"created_by_source_system"`
	FileURL               *string                `json:"file_url"`
	CreatedAt             time.Time              `json:"created_at"`

	ChangedByUser *PrincipalResp `json:"changed_by_user,omitempty"`
}

type PrincipalResp struct {
	ExternalID   string `json:"external_id"`
	SourceSystem string `json:"source_system"`
	Email        string `json:"email"`
	DisplayName  string `json:"display_name"`
	Username     string `json:"username"`
	Profile      string `json:"profile"`
	Group        string `json:"group"`
	Role         string `json:"role"`
	Status       string `json:"status"`
}
